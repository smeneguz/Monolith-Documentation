...
$db = new database();
// WHERE usecase  LIKE \"%-%\"
$ans = $db->query("SELECT * FROM ValidationTests");

$data = array();
while($res = $ans->fetch_assoc()){
   $test = 'TV' . substr(printRequirement($res['requirement']),1);
...
usort($ks,"compare_uc");
$FH = fopen($pathOutPdQ."validation_test.tex", "w");
if($FH ===false){
   die("ERRORE FILE");
}
...
fwrite($FH, fineTabella('Test di validazione'));
fclose($FH);
